// Streaming speech-to-text over a local faster-whisper WebSocket server.
//
...
//   - 'auto'   → faster-whisper if stt.fasterWhisperURL is set; else unavailable (batch fallback)
//   - 'faster-whisper' → force faster-whisper (available iff URL set)
//   - 'batch'          → unavailable (the main process falls back to the legacy flush loop)
...
// Encode one frame (RFC 6455 §5.2). `mask` should be true for client→server frames (the spec
...
module.exports = {
  createStreamSTT,
  // exported for testing / reuse by the reference server in tests:
